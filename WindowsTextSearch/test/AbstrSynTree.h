...
/*
  ...
  ====================
	ver 1.6 : 02 Mar 2017
	- added path data to declarations
	- changed statements vector to hold containing file path information
	- changed public data to private
...
namespace CodeAnalysis
{
  ...
  enum DeclType 
  { 
    dataDecl, functionDecl, lambdaDecl, usingDecl, typedefDecl, enumVarDecl
  };

  struct DeclarationNode
  {
		Scanner::ITokCollection*& pTc() { return pTc_; }
		Access& access() { return access_; }
		DeclType& declType() { return declType_; }
		std::string& package() { return package_; }
		std::string& path() { return path_; }
		size_t& line() { return line_; }
	private:
    Scanner::ITokCollection* pTc_ = nullptr;
    Access access_;
    DeclType declType_;
    std::string package_;
		std::string path_;
  ...
  struct ASTNode
  {
    ...
    ~ASTNode();
		...
		Type& parentType() { return parentType_; }
		Name& name() { return name_; }
		Package& package() { return package_; }
		Path& path() { return path_; }
		size_t& startLineCount() { return startLineCount_; }
		size_t& endLineCount() { return endLineCount_; }
		size_t& complexity() { return complexity_; }
		std::vector<ASTNode*>& children() { return children_; }
		std::vector<DeclarationNode>& decl() { return decl_; }
		std::vector<std::pair<Scanner::ITokCollection*, std::string>>& statements() { return statements_; }
    ...
    std::vector<DeclarationNode> decl_;
		std::vector<std::pair<Scanner::ITokCollection*, std::string>> statements_;
  };
  ...
  inline void complexityWalk(ASTNode* pItem, size_t& count)
  {
    ...
    pItem->complexity() = count - inCount + 1;
  }
  //----< compute complexities for each ASTNode >--------------------
