...
#include "../../../SharedProjects/Task/Task.h"
...
*/
template<typename CallObj>
inline bool SocketListener::start(CallObj& co)
{
  if (!bind())
  {
    return false;
  ...
  std::thread ListenThread(
    [&]()
    {
      ...
      while (!acceptFailed_)
      {
        ...
        //Tasks::Start([&co, &clientSocket]{ co(std::move(clientSocket)); });
        //std::function<void(Socket&)> lambda = [&co](Socket& s) {co(s); };
        //std::thread clientThread(lambda, std::move(clientSocket));
