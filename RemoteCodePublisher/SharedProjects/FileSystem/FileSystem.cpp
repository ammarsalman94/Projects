...
void demoFileSystemSearchClass() {
...
void demoFileInfoClassOperations1(int argc, char ** argv) {
	...
	if (fi.good()) {
		...
		std::cout << "\n  size: " << "\t" << fi.size() << " bytes";
		if (fi.isArchive()) std::cout << "\n  is archive";
		else std::cout << "\n  is not archive";
		if (fi.isCompressed()) std::cout << "\n  is compressed";
		else std::cout << "\n  is not compressed";
		if (fi.isDirectory()) std::cout << "\n  is directory";
		else std::cout << "\n  is not directory";
		if (fi.isEncrypted()) std::cout << "\n  is encrypted";
		else std::cout << "\n  is not encrypted";
		if (fi.isHidden()) std::cout << "\n  is hidden";
		else std::cout << "\n  is not hidden";
		if (fi.isNormal()) std::cout << "\n  is normal";
		else std::cout << "\n  is not normal";
		if (fi.isOffLine()) std::cout << "\n  is offline";
		else std::cout << "\n  is not offline";
		if (fi.isReadOnly()) std::cout << "\n  is readonly";
		else std::cout << "\n  is not readonly";
		if (fi.isSystem()) std::cout << "\n  is system";
		else std::cout << "\n  is not system";
		if (fi.isTemporary()) std::cout << "\n  is temporary";
		else std::cout << "\n  is not temporary";
	}
	else
...
void demoFileInfoClassOperations2(int argc, char** argv) {
	std::string fn1;
	std::string fn2;
	if (argc > 2) {
		fn1 = argv[1];
...
void demoFileClassOperations2(int argc, char ** argv) {
...
int main(int argc, char* argv[]) {
	...
	//demoFileSystemSearchClass();
	
	demoDirectoryClass();
	
	demoFileInfoClassOperations1(argc, argv);
	demoFileInfoClassOperations2(argc, argv);
	
	demoFileClassOperations1();
	demoFileClassOperations2(argc, argv);
	demoFileClassOperations3();
	return demoFileClassOperations4();
}
#endif
