...
private:
  std::string type_;
  std::string name_;
  size_t startLineCount_ = 0;
  size_t endLineCount_ = 0;
  std::string path_;
...
void testScopeStack1(ScopeStack<element*>& testStack, element* pItem) {
	...
	(testStack.top()->children()).push_back(pItem);
	testStack.push(pItem);
...
void testScopeStack2(ScopeStack<element*>& testStack, element* pItem) {
...
int main()
{
  ...
  element* pItem = new element;
	...
	testScopeStack1(testStack, pItem);
	testScopeStack2(testStack, pItem);
