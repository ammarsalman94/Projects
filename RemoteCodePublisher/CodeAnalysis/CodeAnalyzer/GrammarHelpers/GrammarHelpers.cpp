...
//----< find last matching between two indexes in semiExp >----------
size_t GrammarHelper::findLastBetween(const Scanner::ITokCollection& se, const std::string& token, size_t i1, size_t i2) {
	if (i2 == 0) return findLast(se, token);
	for (size_t i = i1; i < i2; ++i) {
		std::string debugTok = se[i2 - i];
		if (se[i2 - i] == token)
			return i2 - i;
	}
	return se.length();
...
int main() {
	...
	load(tokset0, se); test(se);
	load(tokset1, se); test(se);
	load(tokset2, se); test(se);
	load(tokset3, se); test(se);
	load(tokset4, se); test(se);
	load(tokset5, se); test(se);
	load(tokset6, se); test(se);
	load(tokset7, se); test(se);
	load(tokset8, se); test(se);
	load(tokset9, se); test(se);
	load(tokset10, se); test(se);
	load(tokset11, se); test(se);
	load(tokset12, se); test(se);
	load(tokset13, se); test(se);
	load(tokset14, se); test(se);
	load(tokset15, se); test(se);
}
#endif
