...
// Application: Project #2, OOD, Sprint 2017                       //
...
#include "../../../SharedProjects/Logger/Logger.h"
#include "../../../SharedProjects/Utilities/Utilities.h"
...
void CodeAnalysisExecutive::displayMetricsLine(const File& file, ASTNode* pNode)
{
  ...
  out << std::setw(12) << pNode->type();
  out << std::setw(35) << trunc(pNode->name(), 33);
  out << std::setw(8) << pNode->startLineCount();
  out << std::setw(8) << pNode->endLineCount() - pNode->startLineCount() + 1;
  size_t debug1 = pNode->startLineCount();
  size_t debug2 = pNode->endLineCount();
  out << std::setw(8) << pNode->complexity();
  Rslt::write(out.str());
}
...
{
  for (auto datum : pNode->decl())
  {
    if (pNode->parentType() == "namespace" || pNode->parentType() == "class" || pNode->parentType() == "struct")
    {
      if (pNode->type() == "function" || pNode->parentType() == "function")
        continue;
      if (datum.access() == Access::publ && datum.declType() == DeclType::dataDecl)
      {
        std::ostringstream out;
        out << std::right;
        out << "\n ";
        ...
        if (isSummary)
        {
          out << datum.package() << " : " << datum.line() << " - "
            << pNode->type() << " " << pNode->name() << "\n " << std::setw(15) << " ";
        }
...
// -----< Process C++ hedaer files code >-------------------------------------
void CodeAnalysis::CodeAnalysisExecutive::processHeaderFiles(bool showProc) {
...
// -----< Process CPP source code files >----------------------------------
void CodeAnalysis::CodeAnalysisExecutive::processCppFiles(bool showProc) {
...
void CodeAnalysis::CodeAnalysisExecutive::processCSFiles(bool showProc) {
...
void CodeAnalysisExecutive::displayMetrics(ASTNode* root)
{
  ...
  std::function<void(ASTNode* pNode)> co = [&](ASTNode* pNode) {
    if (
      pNode->type() == "namespace" ||
      pNode->type() == "function" ||
      pNode->type() == "class" ||
      pNode->type() == "interface" ||
      pNode->type() == "struct" ||
      pNode->type() == "lambda"
      )
      fileNodes_.push_back(std::pair<File, ASTNode*>(pNode->package(), pNode));
  };
...
void CodeAnalysisExecutive::displayMetricSummary(size_t sMax, size_t cMax)
{
  ...
  for (auto item : fileNodes_)
  {
    ...
    {
      size_t size = item.second->endLineCount() - item.second->startLineCount() + 1;
      size_t cmpl = item.second->complexity();
