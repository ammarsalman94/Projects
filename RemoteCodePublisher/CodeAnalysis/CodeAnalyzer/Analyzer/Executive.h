...
*  --------------------
*  ver 1.5 : 02 Mar 2017
*  - divided processSourceFiles code into three functions (for header, cpp and cs files)
*  - added support for returning const reference to const fileMap
*  ver 1.4 : 26 Feb 2016
...
namespace CodeAnalysis
{
  ...
  public:
    ...
    virtual void processSourceCode(bool showProc);
    ...
    void setLogFile(const File& file);
		const FileMap& fileMap() const { return fileMap_; }
  ...
  private:
    ...
    virtual void displayDataLines(ASTNode* pNode, bool isSummary = false);
		virtual void processHeaderFiles(bool showProc);
		virtual void processCppFiles(bool showProc);
		virtual void processCSFiles(bool showProc);
