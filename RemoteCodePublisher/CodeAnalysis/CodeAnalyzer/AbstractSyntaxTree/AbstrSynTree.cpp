...
std::string ASTNode::show(bool details)
{
  ...
  if(name_ != "none")
    temp << name_ << ", ";
  if(type_ == "namespace" || type_ == "class" || type_ == "interface" 
		|| type_ == "struct" || type_ == "function" || type_ == "enum")
    temp << package_ << ", ";
  if (details)
  {
...
AbstrSynTree::AbstrSynTree(ScopeStack<ASTNode*>& stack) : stack_(stack)
{
  ...
  pGlobalNamespace_->startLineCount() = 1;
  pGlobalNamespace_->endLineCount() = 1;
...
{
  pNode->parentType() = stack_.top()->type();
  stack_.top()->children().push_back(pNode);  // add as child of stack top
  stack_.push(pNode);                        // push onto stack
  if (pNode->type() == "class" || pNode->type() == "struct" || pNode->type() == "interface" ||pNode->type() == "enum")
    typeMap_[pNode->name()] = pNode;
}
